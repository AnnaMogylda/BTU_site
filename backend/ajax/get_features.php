<?php
    ...
    $product_id = $h->request->get('product_id', 'integer');
    if(!empty($category_id)) {
        $features = $h->features->get_features(array('category_id'=>$category_id));
    } else {
        $features = $h->features->get_features();
    }

    /*Выборка значений свойств*/
    $options = array();
    if(!empty($product_id)) {
        foreach($h->features_values->get_features_values(array('product_id'=>$product_id)) as $fv) {
