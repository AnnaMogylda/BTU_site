<?php
    ...
    $stock_filter = (!$h->settings->is_preorder ? "AND (pv.stock IS NULL OR pv.stock>0)" : "");
    $h->db->query("SELECT 
            p.id, 
            $px.name, 
            i.filename as image 
    ...
    foreach($h->db->results() as $product) {
        $products[$product->id] = $product;
    }

    /*Выборка вариантов для найденных товаров*/
    $lang_sql = $h->languages->get_query(array('object'=>'variant', 'px'=>'pv'));
    $variants = array();
    if(!empty($products)) {
        $h->db->query("SELECT 
                pv.id, 
        ...
        ", $h->settings->max_order_amount, $h->settings->max_order_amount, array_keys($products));
        $variants = $h->db->results();
    }
    
    foreach($variants as $variant) {
        if(isset($products[$variant->product_id])) {
            $variant->units = $variant->units ? $variant->units : $h->settings->units;
            $products[$variant->product_id]->variants[] = $variant;
            if ($variant->currency_id && ($currency = $h->money->get_currency(intval($variant->currency_id)))) {
