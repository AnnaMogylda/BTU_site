...
$script = $h->request->post('script');
$theme = $h->request->post('theme', 'string');

/*Сохранение скриптов из админки*/
$file = $h->config->root_dir.'design/'.$theme.'/js/'.$script;
...
if(is_file($file) && is_writable($file) && !is_file($h->config->root_dir.'design/'.$theme.'/locked')) {
    ...
    $js_version = ltrim($h->settings->js_version, '0');
    if (!$js_version) {
        $js_version = 0;
    }
    $h->settings->js_version = str_pad(++$js_version, 6, 0, STR_PAD_LEFT);
