...
if (!empty($argv)) {
    ...
    $params['root_url'] = $h->config->root_url;
}
...
if ($params['output'] == 'file') {
    $sub_sitemaps = glob($h->config->root_dir . "/sitemap" . $params['l'] . "_*.xml");
    if (is_array($sub_sitemaps)) {
        foreach ($sub_sitemaps as $sitemap) {
            @unlink($sitemap);
        }
    }
    if (file_exists($h->config->root_dir . "/sitemap" . $params['l'] . ".xml")) {
        @unlink($h->config->root_dir . "sitemap" . $params['l'] . ".xml");
    }
} else {
...
// Страницы
foreach($h->pages->get_pages() as $p) {
    if($p->visible && $p->url && $p->url != '404') {
        $url = $main_url.esc($p->url);
        $last_modify = array();
        if ($p->url == 'blog') {
            $h->db->query("SELECT b.last_modify FROM __blog b");
            $last_modify = $h->db->results('last_modify');
            $last_modify[] = $h->settings->lastModifyPosts;
        }
...
// Блог
$posts_count = $h->blog->count_posts(array('visible'=>1));
foreach($h->blog->get_posts(array('visible'=>1, 'limit'=>$posts_count)) as $p) {
...
// Бренды
$brands_count = $h->brands->count_brands(array('visible_brand'=>1));
foreach($h->brands->get_brands(array('visible_brand'=>1, 'limit'=>$brands_count)) as $b) {
...
// Товары
$h->db->query("SELECT url, last_modify FROM __products WHERE visible=1");
foreach($h->db->results() as $p) {
