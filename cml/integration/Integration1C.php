...
class Integration1C
{
    ...
    public function __construct($h, $start_time) {
        $this->h          = $h;
    ...
    public function check_auth() {
        ...
        if (!$manager_1c = $this->h->managers->get_manager($login)) {
            return false;
        }
        
        if (!$this->h->managers->check_password($pass, $manager_1c->password)) {
            return false;
        }
        
        if (!$this->h->managers->access('integration_1c', $manager_1c)) {
            return false;
        }
    ...
    public function flush_database() {
        $this->h->clear_catalog();
        $this->h->db->query("TRUNCATE TABLE `__orders`");
        $this->h->db->query("TRUNCATE TABLE `__purchases`");
    }
