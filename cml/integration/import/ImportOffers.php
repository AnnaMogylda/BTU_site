...
class ImportOffers extends Import
{
    ...
    private function import_variant($xml_variant) {
        ...
        $this->h->db->query('SELECT v.id FROM __variants v WHERE v.external_id=? AND product_id=(SELECT p.id FROM __products p WHERE p.external_id=? LIMIT 1)', $variant_1c_id, $product_1c_id);
        $variant_id = $this->h->db->result('id');

        $this->h->db->query('SELECT p.id FROM __products p WHERE p.external_id=?', $product_1c_id);
        $variant->external_id = $variant_1c_id;
        $variant->product_id = $this->h->db->result('id');
        ...
        if (empty($variant_id)) {
            $this->h->variants->add_variant($variant);
        } else {
    ...
    private function init_currencies() {
        ...
        foreach ($this->h->money->get_currencies($currency_filter) as $c) {
