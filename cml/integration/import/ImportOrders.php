...
class ImportOrders extends Import
{
    ...
    public function import($xml_file) {
        ...
        foreach ($this->h->orderstatus->get_status() as $s) {
        ...
        foreach($xml->Документ as $xml_order) {
            ...
            $existed_order = $this->h->orders->get_order($order->id);
            ...
            if ($existed_order) {
                $this->h->orders->update_order($order->id, $order);
            } else {
                $order->id = $this->h->orders->add_order($order);
            }
            ...
            foreach ($xml_order->Товары->Товар as $xml_product) {
                ...
                // или попробуем поискать по артикулу
                    ...
                    $this->h->db->query('SELECT id FROM __variants WHERE sku=? AND product_id=?', $sku, $product_id);
                    $variant_id = $this->h->db->result('id');
                // последняя попытка, это если у товара всего один вариант, вероятнее всего он нужен
                    ...
                    $variant_ids = $this->h->db->results('id');
                ...
                if (!empty($variant_id)) {
                    $this->h->db->query("SELECT id FROM __purchases WHERE order_id=? AND product_id=? AND variant_id=?", $order->id, $product_id, $variant_id);
                    $purchase_id = $this->h->db->result('id');
                    if (!empty($purchase_id)) {
                        $purchase_id = $this->h->orders->update_purchase($purchase_id, $purchase);
                    } else {
                        $purchase_id = $this->h->orders->add_purchase($purchase);
                    }
            ...
            foreach ($this->h->orders->get_purchases(array('order_id'=>intval($order->id))) as $purchase) {
                if (!in_array($purchase->id, $purchases_ids)) {
                    $this->h->orders->delete_purchase($purchase->id);
                }
            }

            $this->h->db->query('UPDATE __orders SET discount=0, total_price=? WHERE id=? LIMIT 1', $xml_order->Сумма, $order->id);
        }
