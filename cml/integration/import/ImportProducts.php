...
class ImportProducts extends Import
{
    ...
    public function __construct($h, $integration_1c) {
        parent::__construct($h, $integration_1c);
    }
    ...
    private function import_categories($xml, $parent_id = 0) {
        if(isset($xml->Группы->Группа)) {
            foreach ($xml->Группы->Группа as $xml_group) {
                $this->h->db->query('SELECT id FROM __categories WHERE external_id=?', (string)$xml_group->Ид);
                $category_id = $this->h->db->result('id');
                $name = (string)$xml_group->Наименование;
                if (empty($category_id)) {
                    $category_id = $this->h->categories->add_category(array(
                        'parent_id' => $parent_id,
                        'external_id' => (string)$xml_group->Ид,
                        'url' => $this->h->translit($name),
                        'name' => $name,
                        'meta_title' => $name,
                        'meta_keywords' => $name,
                    ...
                    $this->h->categories->update_category($category_id, array(
                        'parent_id' => $parent_id,
                        'name' => $name
                    ));
                }
    ...
    private function import_features($xml) {
        ...
        foreach ($property as $xml_feature) {
            ...
            if ($xml_feature->Наименование == $this->integration_1c->brand_option_name) {
                ...
                $this->h->db->query('SELECT id FROM __features WHERE external_id=?', strval($xml_feature->Ид));
                $feature_id = $this->h->db->result('id');
                // По умолчанию свойство АКТИВИРУЕМ для фильтра
                if (empty($feature_id)) {
                    // Добавляем свойство и Код 1С
                    $feature_id = $this->h->features->add_feature(array(
                        'name' => strval($xml_feature->Наименование),
                        'external_id' => strval($xml_feature->Ид),
                        'in_filter' => 1
                    ));
                } else {
                    $feature_id = $this->h->features->update_feature($feature_id, array(
                        'name'=>strval($xml_feature->Наименование)
                    ));
                }
    ...
    private function import_product($xml_product) {
        ...
        $this->h->db->query('SELECT id FROM __products WHERE external_id=?', $product_1c_id);
        $product_id = $this->h->db->result('id');
        
        // если не нашли, ищем по артикулу
        if (empty($product_id) && !empty($variant->sku)) {
            $this->h->db->query('SELECT product_id, id FROM __variants WHERE sku=? LIMIT 1', $variant->sku);
            $res = $this->h->db->result();
            if (!empty($res)) {
            ...
            if (!empty($variant_1c_id)) {
                $this->h->db->query('SELECT id FROM __variants WHERE external_id=? AND product_id=?', $variant_1c_id, $product_id);
                $variant_id = $this->h->db->result('id');
            } else {
                $this->h->db->query('SELECT id FROM __variants WHERE product_id=?', $product_id);
                $variant_id = $this->h->db->result('id');
            }
        }
        ...
        if ((string)$xml_product->Статус == 'Удален' || (string)$attributes['Статус'] == 'Удален') {
            if ($product_id !== null && $variant_id !== null) {
                $this->h->variants->delete_variant($variant_id);
                $this->h->db->query('SELECT count(id) as variants_num FROM __variants WHERE product_id=?', $product_id);
                if ($this->h->db->result('variants_num') == 0) {
                    $this->h->products->delete_product($product_id);
                }
            }
            return;
        }
        ...
        if (empty($product_id)) {
            ...
            $product_id = $this->h->products->add_product(array(
                'external_id' => $product_1c_id,
                'url' => $this->h->translit($xml_product->Наименование),
            ...
            if (!empty($products_categories_ids)) {
                foreach ($products_categories_ids as $category_id) {
                    $this->h->categories->add_product_category($product_id, $category_id);
                }
            }
            ...
            if ($this->integration_1c->full_update === true) {
                ...
                $p->url = $this->h->translit($xml_product->Наименование);
                $p->name = $xml_product->Наименование;
                $p->meta_title = $xml_product->Наименование;
                $p->meta_keywords = $xml_product->Наименование;

                $product_id = $this->h->products->update_product($product_id, $p);

                // Обновляем категории товара
                if (!empty($products_categories_ids) && !empty($product_id)) {
                    $query = $this->h->db->placehold('DELETE FROM __products_categories WHERE product_id=?', $product_id);
                    $this->h->db->query($query);
                    foreach ($products_categories_ids as $category_id) {
                        $this->h->categories->add_product_category($product_id, $category_id);
                    }
                }
        ...
        if (empty($variant_id)) {
            ...
            $variant->stock = 0;
            $variant_id = $this->h->variants->add_variant($variant);
        } elseif (!empty($variant_id)) {
            $this->h->variants->update_variant($variant_id, $variant);
        }
        ...
        if (isset($xml_product->Изготовитель->Наименование)) {
            ...
            // Добавим бренд
            // Найдем его по имени
            $this->h->db->query('SELECT id FROM __brands WHERE name=?', $brand_name);
            if (!$brand_id = $this->h->db->result('id')) {
                // Создадим, если не найден
                $brand_id = $this->h->brands->add_brand(array(
                    'name' => $brand_name,
                    'meta_title' => $brand_name,
                    'meta_keywords' => $brand_name,
                    'meta_description' => $brand_name,
                    'url' => $this->h->translit_alpha($brand_name),
                    'visible' => 1
                ));
            }
            if (!empty($brand_id)) {
        ...
        if (!empty($main_info)) {
            $this->h->products->update_product($product_id, $main_info);
        }
    }
        ...
        foreach ($xml_features as $xml_option) {
            ...
            if ($this->integration_1c->get_from_storage($param) !== null) {
                ...
                if (isset($main_category_id) && !empty($feature_id)) {
                    $this->h->features->add_feature_category($feature_id, $main_category_id);
                    
                    $values_transaction = $this->h->db->placehold("INSERT IGNORE INTO `__products_features_values` (`product_id`, `value_id`) VALUES ");
                    $sql_values = array();
                    ...
                    if ($sql_values) {
                        ...
                        $this->h->db->query($values_transaction);
                    }
                }
            }
            ...
            elseif ($this->integration_1c->get_from_storage('brand_option_id') !== null && !empty($xml_option->Значение) && $this->integration_1c->get_from_storage('brand_option_id') == strval($xml_option->Ид)) {
                ...
                if (!empty($brand_id)) {
                    $this->h->products->update_product($product_id, array('brand_id'=>$brand_id));
                }
            }
        }
    }
    ...
    private function get_feature_value_id($feature_id, $value) {
        ...
        $value_id = null;
        $translit = $this->h->translit_alpha($value);

        // Ищем значение с таким транслитом
        if ($fv = $this->h->features_values->get_features_values(array('feature_id'=>$feature_id, 'translit'=>$translit))) {
            $fv = reset($fv);
            $value_id = $fv->id;
        }
        ...
        if (empty($value_id)) {
            ...
            $this->h->db->query("SELECT `to_index_new_value` FROM `__features` WHERE `id`=? LIMIT 1", $feature_id);
            $to_index = $this->h->db->result('to_index_new_value');
            ...
            $value_id = $this->h->features_values->add_feature_value($feature_value);
        }
        return $value_id;
    }
    ...
    private function import_images($xml_product, $product_id) {
        ...
        if (isset($xml_product->ОсновнаяКартинка)) {
            ...
            if (!empty($image) && is_file($this->integration_1c->get_tmp_dir() . $image) && is_writable($this->h->config->original_images_dir)) {
                $this->h->db->query('SELECT id, filename FROM __images WHERE product_id=? AND filename=? ORDER BY position LIMIT 1', $product_id, $image);
                $img_id = $this->h->db->result('id');
                if (!empty($img_id)) {
                    $this->h->products->delete_image($img_id);
                }
                rename($this->integration_1c->get_tmp_dir() . $image, $this->h->config->original_images_get_tmp_dir() . $image);
                $images_ids[] = $this->h->products->add_image($product_id, $image, '', $position++);
            }
        }
        ...
        if (isset($xml_product->Картинка)) {
            foreach ($xml_product->Картинка as $img) {
                ...
                $original_images_dir = $this->h->config->root_dir . $this->h->config->original_images_dir;
                if (!empty($filename) && is_file($this->integration_1c->get_tmp_dir(). $image) && is_writable($original_images_dir)) {
                    $this->h->db->query('SELECT id, filename FROM __images WHERE product_id=? AND filename=? ORDER BY position LIMIT 1', $product_id, $filename);
                    $img_id = $this->h->db->result('id');
                    if (!empty($img_id)) {
                        $this->h->products->delete_image($img_id);
                    }
                    rename($this->integration_1c->get_tmp_dir(). $image, $original_images_dir . $filename);
                    $images_ids[] = $this->h->products->add_image($product_id, $filename, '', $position++);
                }
            }
        }
        ...
        if (!empty($images_ids)) {
            ...
            $this->h->db->query('UPDATE __products SET main_image_id=? WHERE id=? LIMIT 1', $main_image_id, $product_id);
        }
    }
