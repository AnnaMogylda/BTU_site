...
class ExportOrders extends Export
{
    ...
    public function export() {
        ...
        $orders = $this->h->orders->get_orders(array('modified_since'=>$this->h->settings->last_1c_orders_export_date));
        foreach($orders as $order) {
            ...
            $purchases = $this->h->orders->get_purchases(array('order_id'=>intval($order->id)));

            $t1 = $doc->addChild ( 'Товары' );
            foreach($purchases as $purchase) {
                if(!empty($purchase->product_id) && !empty($purchase->variant_id)) {
                    $this->h->db->query('SELECT external_id FROM __products WHERE id=?', $purchase->product_id);
                    $id_p = $this->h->db->result('external_id');
                    $this->h->db->query('SELECT external_id FROM __variants WHERE id=?', $purchase->variant_id);
                    $id_v = $this->h->db->result('external_id');

                    // Если нет внешнего ключа товара - указываем наш id
                    if(!empty($id_p)) {
                        $id = $id_p;
                    } else {
                        $this->h->db->query('UPDATE __products SET external_id=id WHERE id=?', $purchase->product_id);
                    ...
                    if(!empty($id_v)) {
                        ...
                        $this->h->db->query('UPDATE __variants SET external_id=id WHERE id=?', $purchase->variant_id);
            ...
            $payment_method = $this->h->payment->get_payment_method($order->payment_method_id);
            $delivery = $this->h->delivery->get_delivery($order->delivery_id);
