...
$public_key		 	= $h->request->post('public_key');
$amount				= $h->request->post('amount');
$currency			= $h->request->post('currency');
$description		= $h->request->post('description');
$liqpay_order_id	= $h->request->post('order_id');
$order_id			= intval(substr($liqpay_order_id, 0, strpos($liqpay_order_id, '-')));
$type				= $h->request->post('type');
$signature			= $h->request->post('signature');
$status				= $h->request->post('status');
$transaction_id		= $h->request->post('transaction_id');
$sender_phone		= $h->request->post('sender_phone');
...
if($amount != round($h->money->convert($order->total_price, $method->currency_id, false), 2) || $amount<=0)
	die("incorrect price");
...
// header('Location: '.$h->config->root_url.'/order/'.$order->url);
