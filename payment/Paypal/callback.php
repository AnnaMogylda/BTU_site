...
// Get the order
$order = $h->orders->get_order(intval($h->request->post('invoice')));
if(empty($order))
	die('Order not found');
...
if ($h->request->post('receiver_email') != $settings['business']) 
	die("Incorrect merchant email"); 

// Verify transaction type
if ($h->request->post('txn_type') != 'cart') 
...
{			
	$price = $h->money->convert($purchase->price, $method->currency_id, false);
	$price = round($price, 2);
...
{
	$delivery_price = $h->money->convert($order->delivery_price, $payment_method->currency_id, false);
	$delivery_price =round($delivery_price, 2);
	$total_price += $delivery_price;
}
if($total_price != $h->request->post('mc_gross'))
	die("Incorrect total price (".$total_price."!=".$h->request->post('mc_gross').")");
